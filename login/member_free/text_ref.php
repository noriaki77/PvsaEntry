...
if ($keyword != "") {
$sql = "SELECT id,data26,data27,data28,data29 FROM data ";
$sql.= "WHERE data29 LIKE '%" . $keyword . "%' ORDER BY id";
}
if ($text != "") {
$sql = "SELECT id,data26,data27,data28,data29 FROM data ";
$sql.= "WHERE data28 LIKE '%" . $text . "%' ORDER BY id";
}

} else {

if ($keyword != "") {
$sql = "SELECT id,data26,data27,data28,data29 FROM data "; // SQL文
$sql.= "WHERE data29 LIKE '%" . $keyword . "%' ";
$sql.= "ORDER BY " . $order . "";
}
if ($text != "") {
$sql = "SELECT id,data26,data27,data28,data29 FROM data "; // SQL文
$sql.= "WHERE data28 LIKE '%" . $text . "%' ";
$sql.= "ORDER BY " . $order . "";
}

}

//$sql = "SELECT id,number,title,filename FROM upload"; // SQL文
...
//print("<div align='right'><A HREF='excel.php' TARGET='_blank' class='button blue'>Excel形式保存</A></div>");
//print("<table border='0'><tr><td align='left' valign='bottom'>".$total."件中 ".$from."〜".$to."件を表示");
//print("<div align='right'><A HREF='excel.php' TARGET='_blank' class='button blue'>Excel保存</A></div></td></tr></table>");
...
//for($i=0; $i<$num; $i++) {
 print("<TH>受付番号<font size=1><a href=\"text_ref.php?order=".$order2."\">▲</a><a href=\"text_ref.php?order=".$order1."\">▼</a></font></TH>");
 print("<TH>表題</TH>");
 //print("<TH>表題<font size=1><a href=\"admin_regi.php?order=".$order4."\">▲</a><a href=\"admin_regi.php?order=".$order3."\">▼</a></font></TH>");
 print("<TH>著者</TH>");
 print("<TH>要約</TH>");
 print("<TH>キーワード</TH>");
//}
...
//while($row = mysql_fetch_array($result)) {
  print("<TR>");
  for($j=0; $j<1; $j++) {
    print("<TD width='15%'><a href=\"text_ref_ctrl.php?number=".$row[$j]."&keyword=". urlencode($keyword)."&text=". urlencode($text)."\">".$row[$j]."</a></TD>");
  }
  for($j=1; $j<3; $j++) {
  //for($j=1; $j<$num; $j++) {
    print("<TD width='15%'>".$row[$j]."</TD>");
  }
  for($j=3; $j<4; $j++) {
  //文字コード設定
  $str1 = $row[$j];
  $substr1 = mb_substr($str1, 0, 50,"utf-8");
    print("<TD>".$substr1."</TD>");
  }
  for($j=4; $j<5; $j++) {
    print("<TD width='15%'>".$row[$j]."</TD>");
...
//HTML文を出力　.$pageに指定された頁数をセットしてGETで渡すリンクを作成
	print("<a href='text_ref.php?page=".$i ."'>&#171; Previous</a>" );
}
...
//HTML文を出力　.$pageに指定された頁数をGETで渡すリンクを作成
		print("<a href='text_ref.php?page=".$i."'>$i</a>");
	}
//最終頁以外では頁間を区切る「・」を出力
//	if ($i <> $maxpage) {
//		print("・");
...
if ($page < $maxpage) {
			$i = $page + 1;
//HTML文を出力　.$pageに指定された頁数をセットしてGETで渡すリンクを作成
			print( "<a href='text_ref.php?page=".$i."'>Next &#187;</a>");
}
