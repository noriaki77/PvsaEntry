...
if ($deny_flag) {
	...
	} elseif (isset($_POST['page']) && ($_POST['page'] === 'page1') && !$global_error_flag) {
		// 入力画面
		$page = 'page1';
...
$id = $_SESSION['id'];
$sql4 = "SELECT * FROM data "; // SQL文
$sql4.= "WHERE id = '" . $id . "'";
$result4 = executeQuery($sql4);
$row4 = mysql_fetch_array($result4);
$day = $row4[1];
$subday = substr($day, 0, 10);
$mainday = strtotime($subday);
...
$id = $_SESSION['id'];
		...
		*/
		...
		$data27 = $db_value[1] ;
		$data28 = $db_value[2] ;
		$data28 = htmlspecialchars($data28) ;
if ( get_magic_quotes_gpc() ) {
  $data28 = stripslashes( $data28 );
}
		$data29 = $db_value[3] ;
		//add to the database
		$result_sql = mysql_query("UPDATE data SET 
data25 = '$data25',
data26 = '$data26',
data27 = '$data27',
data28 = '$data28'
...
$id = $_SESSION['id'];
	...
	$tmpl->set('value3', $value3);
	$tmpl->set('id', $id);
//	$tmpl->set('mainday', $mainday);
//	$tmpl->set('subday', $subday);
	// テンプレート書き出し
...
*/
	...
	$tmpl->set('id', $id);
/*
	$tmpl->set('checked0', $checked0);
	$tmpl->set('checked1', $checked1);
	$tmpl->set('会員ID', $会員ID);
	$tmpl->set('メールアドレス', $メールアドレス);
	$tmpl->set('お名前（姓）', $お名前（姓）);
	$tmpl->set('お名前（名）', $お名前（名）);
	$tmpl->set('フリガナ（セイ）', $フリガナ（セイ）);
	$tmpl->set('フリガナ（メイ）', $フリガナ（メイ）);
	$tmpl->set('郵便番号', $郵便番号);
	$tmpl->set('都道府県', $都道府県);
	$tmpl->set('以降の住所', $以降の住所);
	$tmpl->set('電話番号', $電話番号);
	$tmpl->set('参加者１：お名前（姓）', $参加者１：お名前（姓）);
	$tmpl->set('参加者１：お名前（名）', $参加者１：お名前（名）);
	$tmpl->set('参加者１：お名前（セイ）', $参加者１：お名前（セイ）);
	$tmpl->set('参加者１：お名前（メイ）', $参加者１：お名前（メイ）);
	$tmpl->set('参加者２：お名前（姓）', $参加者２：お名前（姓）);
	$tmpl->set('参加者２：お名前（名）', $参加者２：お名前（名）);
	$tmpl->set('参加者２：お名前（セイ）', $参加者２：お名前（セイ）);
	$tmpl->set('参加者２：お名前（メイ）', $参加者２：お名前（メイ）);
	$tmpl->set('参加者３：お名前（姓）', $参加者３：お名前（姓）);
	$tmpl->set('参加者３：お名前（名）', $参加者３：お名前（名）);
	$tmpl->set('参加者３：お名前（セイ）', $参加者３：お名前（セイ）);
	$tmpl->set('参加者３：お名前（メイ）', $参加者３：お名前（メイ）);
*/
	$tmpl->set('表題', $表題);
	$tmpl->set('著者', $著者);
	$tmpl->set('要約', $要約);
	$tmpl->set('キーワード', $キーワード);
