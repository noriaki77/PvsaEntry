...
if ($order == "") {
$sql = "SELECT id,time,data0,data1,data3 FROM data";
} else {
$sql = "SELECT id,time,data0,data1,data3 FROM data "; // SQL文
$sql.= "ORDER BY " . $order . "";
}
...
//for($i=0; $i<$num; $i++) {
 print("<TH>受付番号<font size=1><a href=\"admin_regi.php?order=".$order2."\">▲</a><a href=\"admin_regi.php?order=".$order1."\">▼</a></font></TH>");
 //print("<TH>受付日時</TH>");
 print("<TH>受付日時<font size=1><a href=\"admin_regi.php?order=".$order4."\">▲</a><a href=\"admin_regi.php?order=".$order3."\">▼</a></font></TH>");
 print("<TH>会員区分</TH>");
 print("<TH>会員ID</TH>");
...
//while($row = mysql_fetch_array($result)) {
  print("<TR>");
  for($j=0; $j<1; $j++) {
    print("<TD><a href=\"admin_regi_ctrl.php?number=".$row[$j]."\">".$row[$j]."</a></TD>");
  }
  //for($j=1; $j<$num-1; $j++) {
  for($j=1; $j<$num; $j++) {
    print("<TD>".$row[$j]."</TD>");
...
//HTML文を出力　.$pageに指定された頁数をセットしてGETで渡すリンクを作成
	print("<a href='admin_regi.php?page=".$i ."'>&#171; Previous</a>" );
}
...
//HTML文を出力　.$pageに指定された頁数をGETで渡すリンクを作成
		print("<a href='admin_regi.php?page=".$i."'>$i</a>");
	}
//最終頁以外では頁間を区切る「・」を出力
//	if ($i <> $maxpage) {
//		print("・");
...
if ($page < $maxpage) {
			$i = $page + 1;
//HTML文を出力　.$pageに指定された頁数をセットしてGETで渡すリンクを作成
			print( "<a href='admin_regi.php?page=".$i."'>Next &#187;</a>");
}
