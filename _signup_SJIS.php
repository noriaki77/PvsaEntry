...
if(isset($_POST['signup'])){
	...
	if (!mb_ereg_match('/^[ァ-ヶー]+$/', $name4) && $name4 != "" ) {
	$action['result'] = 'error'; array_push($text,'フリガナ（名）は全角カナで入力して下さい。');
	}
	echo $kana1;
	
	if(!preg_match("/^\d{7}$/", $yubin) && $yubin != "" ){
	$action['result'] = 'error'; array_push($text,'郵便番号は半角数字(ハイフンなし)で入力して下さい。');
	}
...
<tr>
	...
	<td>姓<input type="text" size="12" name="kana1" value="<?= $_POST['kana1'] ?>" />
		名<input type="text" size="12" name="name4" value="<?= $_POST['name4'] ?>" />（例：ヤマダ　タロウ）
	</td>
</tr>
<tr>
	<th>郵便番号（半角数字）</th>
	<td><input type="text" name="yubin" size="10" maxlength="8" value="<?= $_POST['yubin'] ?>" />（例：1001111）
	<input type="button" onclick="AjaxZip3.zip2addr(yubin,'','pref','jusho');" value="住所を検索" title="ボタンを押すと、郵便番号から住所を検索して表示します。" />
	<br />
