...
if ($order == "") {
$sql = "SELECT username,email,data,data1,data2 FROM users_ref";
} else {
$sql = "SELECT username,email,data,data1,data2 FROM users_ref "; // SQL文
$sql.= "ORDER BY " . $order . "";
}
...
//for($i=0; $i<$num; $i++) {
 //print("<TH>登録番号<font size=1><a href=\"admin_ref.php?order=".$order2."\">▲</a><a href=\"admin_ref.php?order=".$order1."\">▼</a></font></TH>");
 //print("<TH>受付日時</TH>");
 print("<TH>会員ID<font size=1><a href=\"admin_ref.php?order=".$order4."\">▲</a><a href=\"admin_ref.php?order=".$order3."\">▼</a></font></TH>");
 print("<TH>メールアドレス</TH>");
 print("<TH>法人名&nbsp;&frasl;&nbsp;病院・施設名</TH>");
 print("<TH>名前（姓）</TH>");
 print("<TH>名前（名）</TH>");
//}
print("</TR>");
...
//while($row = mysql_fetch_array($result)) {
  print("<TR>");
  for($j=0; $j<1; $j++) {
    print("<TD><a href=\"admin_ref_ctrl.php?number=".$row[$j]."\">".$row[$j]."</a></TD>");
  }
  //for($j=1; $j<$num-1; $j++) {
  for($j=1; $j<$num; $j++) {
    print("<TD>".$row[$j]."</TD>");
  ...
  //for($j=4; $j<$num; $j++) {
  //  print("<TD><a href=\"upload/upload/".$row[$j]."\">".$row[$j]."</a></TD>");
  //}
  print("</TR>");
...
//HTML文を出力　.$pageに指定された頁数をセットしてGETで渡すリンクを作成
	print("<a href='admin_ref.php?page=".$i ."'>&#171; Previous</a>" );
}
...
//HTML文を出力　.$pageに指定された頁数をGETで渡すリンクを作成
		print("<a href='admin_ref.php?page=".$i."'>$i</a>");
	}
//最終頁以外では頁間を区切る「・」を出力
//	if ($i <> $maxpage) {
//		print("・");
...
if ($page < $maxpage) {
			$i = $page + 1;
//HTML文を出力　.$pageに指定された頁数をセットしてGETで渡すリンクを作成
			print( "<a href='admin_ref.php?page=".$i."'>Next &#187;</a>");
}
