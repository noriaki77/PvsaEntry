...
<!--パンくずリストここから-->
<div class="pan"><a href="index.php">トップ</a> ＞ ファイルアップ管理</div>
<!--パンくずリストここまで-->

<div class="category">
<h2 class="heading2">ファイルアップ管理</h2>
...
if ($order == "") {
$sql = "SELECT id,userid,number,title,filename FROM upload";
} else {
$sql = "SELECT id,userid,number,title,filename FROM upload "; // SQL文
$sql.= "ORDER BY " . $order . "";
}
...
//for($i=0; $i<$num; $i++) {
 print("<TH>No.<font size=1><a href=\"admin_upload.php?order=".$order2."\">▲</a><a href=\"admin_upload.php?order=".$order1."\">▼</a></font></TH>");
 print("<TH>ユーザID</TH>");
 print("<TH>受付番号<font size=1><a href=\"admin_upload.php?order=".$order4."\">▲</a><a href=\"admin_upload.php?order=".$order3."\">▼</a></font></TH>");
 print("<TH>タイトル</TH>");
 print("<TH>ファイル名</TH>");
//}
print("</TR>");
...
//while($row = mysql_fetch_array($result)) {
  print("<TR>");
  for($j=0; $j<1; $j++) {
    print("<TD><a href=\"admin_upload_ctrl.php?number=".$row[$j]."\">".$row[$j]."</a></TD>");
  }
  for($j=1; $j<$num-1; $j++) {
    print("<TD>".$row[$j]."</TD>");
  }
...
//HTML文を出力　.$pageに指定された頁数をセットしてGETで渡すリンクを作成
	print("<a href='admin_upload.php?page=".$i ."'>&#171; Previous</a>" );
}
...
//HTML文を出力　.$pageに指定された頁数をGETで渡すリンクを作成
		print("<a href='admin_upload.php?page=".$i."'>$i</a>");
	}
//最終頁以外では頁間を区切る「・」を出力
//	if ($i <> $maxpage) {
//		print("・");
...
if ($page < $maxpage) {
			$i = $page + 1;
//HTML文を出力　.$pageに指定された頁数をセットしてGETで渡すリンクを作成
			print( "<a href='admin_upload.php?page=".$i."'>Next &#187;</a>");
}
